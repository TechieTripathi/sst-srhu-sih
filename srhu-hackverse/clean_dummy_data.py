...
Removes all dummy test data, test teams, mock users, test assignments,
and test OTP sessions while preserving the official system configuration:
- Super Admin (superadmin@srhu.edu.in)
- Default Admin (admin@srhu.edu.in)
- 25 Official Internal Jury Accounts across Panels 1-5
...
def clean_dummy_data():
    ...
    otp_col = get_otp_sessions_collection()
    ...
    # 4. Clean test judge assignments
    del_assign = assignments_col.delete_many({})
    print(f"[OK] Cleared {del_assign.deleted_count} test judge assignments.")

    # 5. Clean test OTP sessions
    del_otp = otp_col.delete_many({})
    print(f"[OK] Cleared {del_otp.deleted_count} test OTP sessions.")

    # 6. Clean test evaluations and scores
    ...
    # 7. Clean test audit logs (keep system activity clean)
    ...
    print(f"  - Active OTP Sessions:        {otp_col.count_documents({})}")
